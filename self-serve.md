...
- `floor_for()` puts an open poll in the `polls` room and the cooperative's
  business on the floor, and its debate chamber inherits the server's own
  permissions rather than being locked to the cooperative. A vote posted where
  the people it is open to cannot read it is not open.
