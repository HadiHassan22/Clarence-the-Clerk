...
def opening_pass():
    ...
    """
    return not _load().get("started_at")
...
def said(key):
    return key in _load().get("said", {})


def mark_said(key, now=None):
...
def muted(user_id):
    return str(user_id) in _load().get("muted", [])


def set_muted(user_id, on=True):
...
def nudges_due(bills, roll_for, now=None):
    ...
    for bill in bills:
        ...
        for user_id in roll_for(bill):
            if str(user_id) in ballots or muted(user_id):
                continue
            if said(nudge_key(bill, user_id)):
                continue
...
def away_changes(members, reason_for):
